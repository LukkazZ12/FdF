...
int	main(int argc, char	*argv[])
{
	...
	while (aux_rows >= 0)
	{
		map[aux_rows - 1] = malloc(sizeof(t_point) * (cols + 1));
		ft_bzero((t_point *)&map[aux_rows - 1][cols], 16);
		aux_rows--;
	}
	aux_rows = 0;
	...
	while (aux_rows < rows)
	{
		while (aux_cols < cols)
		{
			mlx_pixel_put(mlx_ptr, win_ptr, map[aux_rows][aux_cols].x, map[aux_rows][aux_cols].y, 0xFFFFFF);
			aux_cols++;
		}
		aux_cols = 0;
		aux_rows++;
